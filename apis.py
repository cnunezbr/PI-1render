...
import operator  # importo modulo operator
import pyarrow as pa  # importo modulo pyarrow.parquet.ParquetFile
...
def userdata(user_id):
    ...
    cantidad_dinero = df_gastos_items[df_gastos_items["user_id"] == user_id][
        "price"
    ].iloc[0]
    # Busca el count_item para el usuario de interés
    count_items = df_gastos_items[df_gastos_items["user_id"] == user_id][
        "items_count"
    ].iloc[0]
...
def countreviews(fecha_inicio, fecha_fin):
    ...
    user_data_entre_fechas = df_reviews[
        (df_reviews["reviews_date"] >= fecha_inicio)
        & (df_reviews["reviews_date"] <= fecha_fin)
    ]
    ...
    porcentaje_recomendaciones = (
        total_recomendaciones_True / total_recomendacion
    ) * 100
...
def userforgenre(genero):
    ...
    top_users = (
        data_por_genero.groupby(["user_url", "user_id"])["playtime_horas"]
        .sum()
        .nlargest(5)
        .reset_index()
    )
    ...
    for index, row in top_users.iterrows():
        ...
        user_info = {"user_id": row["user_id"], "user_url": row["user_url"]}
...
def developer(desarrollador):
    ...
    cantidad_gratis_por_año = (
        data_filtrada[data_filtrada["price"] == 0.0]
        .groupby("release_date")["item_id"]
        .count()
    )
    # Calcula el porcentaje de elementos gratis por año
    porcentaje_gratis_por_año = (
        (cantidad_gratis_por_año / cantidad_por_año * 100).fillna(0).astype(int)
    )
...
def sentiment_analysis(anio):
    ...
    anio_reviews = df_reviews[df_reviews["release_date"] == anio]
...
def recomendacion_usuario(user):
    ...
    best = (
        []
    )  # Lista para almacenar los juegos mejor calificados por usuarios similares
